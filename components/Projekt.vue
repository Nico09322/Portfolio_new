    ...
    >
        ...
        >
            ...
            <div class="w-full rounded-lg overflow-hidden"> 
                <img :src="project.Hero.url" class="w-full h-full object-cover" />
            </div>
            <div class="w-full h-[2rem] flex items-center justify-center group-hover:bg-sky-500 rounded-lg group-hover:text-white text-white/30 bg-white/10 mt-[0.5rem] cursor-pointer duration-75 select-none" @click="moreClicked = true" v-motion
                :initial="{scale:1}"
                :hovered="{scale:1.05}"
                :tapped="{scale:0.8}"
            >mehr Infos</div>
            ...
            <div class=" bg-white/10 backdrop-blur rounded-lg border border-gray-500 flex flex-col items-start justify-center p-[1rem] shadow-[0px_4px_12px_0px_rgba(0,_0,_0,_0.3)] z-50" >
                <div class="bg-white/30 w-full h-[1.5rem] absolute top-0 left-0 rounded-t-lg flex items-center pl-[0.5rem]">
                    <div class="grid grid-cols-3 grid-rows-1 gap-[0.5rem] group">
                        <div class="w-[1rem] h-[1rem] bg-red-500 rounded-full flex justify-center items-center" @click="moreClicked=false"><Icon name="maki:cross" size=".5rem" class="text-red-950 opacity-0 group-hover:opacity-100"/></div>
                        <div class="w-[1rem] h-[1rem] bg-yellow-500 rounded-full flex justify-center items-center" @click="moreClicked=false"><Icon name="typcn:minus" size=".75rem" class="text-yellow-900 opacity-0 group-hover:opacity-100"/></div>
                ...
                <div class="w-full h-full flex flex-col mt-[1rem] font-custom p-[0.5rem] max-h-[60vh] overflow-auto">
                    ...
                    <div class="grid grid-cols-2  w-full h-full gap-[0.5rem]"><img v-for="(pic, index) in project.Pictures" :key="index" :src="pic.url" class="rounded-lg"/></div>
